...
- **Rich Organization Profiles**: View detailed profiles for event organizers, including follower counts, account type (Corporate, NGO, Community), and verification status with distinct badges for corporate and community partners.
...
- **The Prompt**: A carefully crafted prompt instructs the Gemini model to act as an "AI event search assistant" and return a list of event objects in a specific JSON format based on the user's natural language query. It is enhanced with real event data from the application to improve accuracy.
